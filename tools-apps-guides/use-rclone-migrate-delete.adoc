...
summary: 'Rclone est un outil de ligne de commande et un client gratuits pour les opérations S3. Vous pouvez utiliser rclone pour migrer, copier et supprimer des données d"objet sur StorageGRID.' 
---
...
[role="lead"]
_Par Siegfried Hepp et Aron Klein_
...
Pour installer rclone sur un poste de travail ou un serveur, téléchargez-le depuis https://rclone.org/downloads/["rclone.org"^].



=== Étapes de configuration initiale

. Créez le fichier de configuration rclone en exécutant le script de configuration ou en créant manuellement le fichier.
. Dans cet exemple, j'utilise sgdemo pour le nom du terminal StorageGRID S3 distant dans la configuration rclone.
+
.. Créez le fichier de configuration ~/.config/rclone/rclone.conf
+
[source, console]
----
    [sgdemo]
...
----
.. Exécutez la configuration rclone
+
[]
====
# rclone config
...
NOTE: Utilisez --no-check-certificate si vous devez ignorer les certificats SSL.
...
NOTE: Utilisez --Progress ou -P pour afficher la progression de la tâche. Sinon, il n'y a pas de sortie.
